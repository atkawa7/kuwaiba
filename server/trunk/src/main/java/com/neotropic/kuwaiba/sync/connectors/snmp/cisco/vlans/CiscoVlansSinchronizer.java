...
public class CiscoVlansSinchronizer {
    ...
    public List<SyncFinding> execute () throws MetadataObjectNotFoundException, 
    ...
    private void createVlans() throws MetadataObjectNotFoundException, 
            ...
            OperationNotPermittedException, ApplicationObjectNotFoundException
    {
        ...
        for(int i = 0; i < vlanInstances.size(); i++){
            ...
            if (currentVlan == null){
                ...
                long newVlanId = bem.createSpecialObject(Constants.CLASS_VLAN, className, id, attributes, -1);
                currentVlans.add(bem.getObject(newVlanId));
     ...
     * 
     * @throws BusinessObjectNotFoundException the object (port or vlan) doesn't exists
     * @throws OperationNotPermittedException
     * @throws MetadataObjectNotFoundException
     */
    public void relatePorts() throws BusinessObjectNotFoundException, 
            OperationNotPermittedException, MetadataObjectNotFoundException
    {
        ...
        //create a map with ports and vlans, but we avoid the paterns with ff:ff:ff:ff or Xf:ff:ff:ff or ff:ff:ff:fX
                ...
                if(!allVlans.isEmpty()){
                    ...
                    if(currentPort == null){
                        ...
                        findings.add(new SyncFinding(SyncFinding.EVENT_INFO, 
                               "This port doesn't exists, try to run the entity/if Mib provider sync, if error persist check the snmp data ", ciscoMib.toString()));
                    }else{
                        ...
                        List<BusinessObjectLight> assosiatedVlans = bem.getSpecialAttribute(currentPort.getClassName(), currentPort.getId(), RELATIONSHIP_PORT_BELONGS_TO_VLAN);
                        removeUnsyncVlans(currentPort, candidateVlans, assosiatedVlans);
     ...
     * @throws MetadataObjectNotFoundException 
     */
    private void removeUnsyncVlans(BusinessObjectLight port, 
            ...
            throws BusinessObjectNotFoundException, MetadataObjectNotFoundException
    {
        for (BusinessObjectLight assosiatedVlan : assosiatedVlans) {
            ...
            if(!isAssociatedVlanInVlansToAssosiate){
                ...
                String toString = String.format("the relationship between %s and %s was release", port, assosiatedVlan.getName());
