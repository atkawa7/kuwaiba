...
//                        rol = KuwaibaClient.getInstance().getObjectLight(classInfo.getClassName(), Long.valueOf(objectId));
    ...
    public FormLoader load(byte[] structure) {
        ...
        if (formid != null) {
            ...
            */
            String address = Page.getCurrent().getWebBrowser().getAddress();
            RemoteSession remoteSession = (RemoteSession) Page.getCurrent().getUI().getSession().getAttribute("session");
            WebserviceBeanLocal wsBean = (WebserviceBeanLocal) Page.getCurrent().getUI().getSession().getAttribute("wsBean");
            
//            String filePath = Variable.FORM_RESOURCE_STRUCTURES + "/" + formid + ".xml";
//            File file = new File(filePath);
//            byte [] byteArray = XMLUtil.getFileAsByteArray(file);
            try {
                List<RemoteForm> remoteForms = wsBean.getForms(address, remoteSession.getSessionId());
                
                for (RemoteForm remoteForm : remoteForms) {
                    if (formid != null && formid.equals(remoteForm.getName()) && remoteForm.getStructure() != null) {
                        
                        FormLoader formLoader = new FormLoader(remoteForm.getStructure());
                        formLoader.build();
