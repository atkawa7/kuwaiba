...
package org.kuwaiba.apis.web.gui.actions.miniapps;
...
public abstract class AbstractMiniApplication<D, E> {
    ...
    /**
     * The type of mini application. See TYPE_XXX for possible values
     */
    private int type;

    /**
     * Default constructor. Use [AbstractMiniApplicationSubclass].getConstructor(Properties.class, int).newInstance(..., ...) to create instances using reflection
     * @param inputParameters Input configuration parameters for the current mini application
     * @param type Th type of mini application. See TYPE_XX for possible values
     */
    public AbstractMiniApplication(Properties inputParameters, int type) {
