...
/**
 * This action allows the user to explore the existing VLANs and the objects related to them
 * @author Charles Edward Bedon Cortazar <charles.bedon@kuwaiba.org>
 */
public class ShowVLANExplorerAction extends AbstractAction {
    
    
    public ShowVLANExplorerAction(WebserviceBean wsBean) {
        super("VLAN Explorer", wsBean);
    }

    @Override
    public void actionPerformed() {
        Window wdwVLANExplorer = new Window(getCaption());
        VerticalLayout lytContent = new VerticalLayout();

        Grid<RemoteObjectLight> tblVLANs = new Grid<>();
        tblVLANs.addColumn(RemoteObjectLight::getName).setCaption("Name");
        tblVLANs.addColumn((currentVLAN) -> {
            try {
                List<RemoteObjectLight> relatedObjects = wsBean.getSpecialAttribute(currentVLAN.getClassName(), currentVLAN.getId(), "uses", Page.getCurrent().getWebBrowser().getAddress(), 
        ...
        tblVLANs.addColumn((currentVLAN) -> {
            try {
                RemoteObjectLight parent = wsBean.getParent(currentVLAN.getClassName(), currentVLAN.getId(), Page.getCurrent().getWebBrowser().getAddress(), 
                            ((RemoteSession) UI.getCurrent().getSession().getAttribute("session")).getSessionId()); //NOI18N
            ...
            public Collection<AutocompleteSuggestion> querySuggestions(AutocompleteQuery query) {
                try {
                    ...
                    List<RemoteObjectLight> suggestedObjects = wsBean.getSuggestedObjectsWithFilter(query.getTerm(), 
                            Constants.CLASS_VLAN, 15, Page.getCurrent().getWebBrowser().getAddress(), 
        ...
        Button btnSearch = new Button(VaadinIcons.SEARCH, (e) -> {
            try {
                ...
                if (txtFilter.getValue().isEmpty())
                    suggestedObjects = wsBean.getObjectsOfClassLight(Constants.CLASS_VLAN, -1, Page.getCurrent().getWebBrowser().getAddress(), 
                            ((RemoteSession) UI.getCurrent().getSession().getAttribute("session")).getSessionId());
                else
                    suggestedObjects = wsBean.getSuggestedObjectsWithFilter(txtFilter.getValue(), 
                        Constants.CLASS_VLAN, -1, Page.getCurrent().getWebBrowser().getAddress(), 
                            ((RemoteSession) UI.getCurrent().getSession().getAttribute("session")).getSessionId());

                if (suggestedObjects.isEmpty())
                    Notifications.showInfo("Your search has 0 results");
                else
                    tblVLANs.setItems(suggestedObjects);
            } catch (ServerSideException ex) {
                Notifications.showError(ex.getLocalizedMessage());
            }
        });
        ...
        lytContent.addComponents(lytFilter, tblVLANs);
        lytContent.setExpandRatio(tblVLANs, 9.5f);
