...
public class Kuwaiba {
     ...
     * @return success or failure
     * @throws Exception if something goes wrong or the requested operation can't be performed
     */
    ...
    public Boolean relateResourceToService(@WebParam(name="resourceClassName")String resourceClassName,
        ...
        try{
            ...
            Class myResourceClass = sbr.getClassFor(resourceClassName);
            if (myResourceClass == null)
                throw new ClassNotFoundException(resourceClassName);

            Class myServiceClass = sbr.getClassFor(serviceClassName);
            if (myServiceClass == null)
                throw new ClassNotFoundException(serviceClassName);

            return sbr.relateResourceToService(myResourceClass, resourceId, myServiceClass, serviceId);
        }catch(Exception e){
            Level level = Level.SEVERE;
            if (e instanceof InventoryException)
                level = ((InventoryException)e).getLevel();
     ...
     /**
     * Removes the relationship between a resource and a service
     * @param resourceClassName  Resource's class name
     * @param resourceId Resource's id
     * @param serviceClassName Service class name
     * @param serviceId Service id
     ...
     * @throws Exception if something goes wrong or the requested operation can't be performed
     */
    @WebMethod(operationName = "unrelateResourceFromService")
    public Boolean unrelateResourceFromService(@WebParam(name="resourceClassName")String resourceClassName,
            @WebParam(name="resourceId")Long resourceId,
            @WebParam(name = "serviceClassName")String serviceClassName,
            @WebParam(name="serviceId")Long serviceId,
            @WebParam(name = "sessionId")String sessionId) throws Exception{
        try{
            sbr.validateCall("unrelateResourceFromService", getIPAddress(), sessionId);
            Class myResourceClass = sbr.getClassFor(resourceClassName);
            if (myResourceClass == null)
                throw new ClassNotFoundException(resourceClassName);
            ...
            return sbr.unrelateResourceFromService(myResourceClass, resourceId, myServiceClass, serviceId);
        }catch(Exception e){
            Level level = Level.SEVERE;
            if (e instanceof InventoryException)
                level = ((InventoryException)e).getLevel();
