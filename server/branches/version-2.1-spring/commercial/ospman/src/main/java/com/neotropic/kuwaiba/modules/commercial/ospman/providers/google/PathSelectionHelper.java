...
public class PathSelectionHelper {
    ...
    private final BiConsumer<List<BusinessObjectViewEdge>, Runnable> callbackPathSelectionComplete;
        ...
        edges.forEach(edge -> {
            ...
            Registration clickRegistration = edge.addPolylineClickListener(event -> {
                if (!path.containsKey(edge.getViewEdge())) {
                    ...
                    tmpPolyline.setStrokeWeight(MapConstants.EDGE_STROKE_WEIGHT);
                    path.put(edge.getViewEdge(), tmpPolyline);
                    
                    tmpPolyline.setPath(edge.getPath());
                    tmpPolyline.addPolylineMouseOverListener(mouseOverEvent -> tmpPolyline.setStrokeWeight(MapConstants.EDGE_STROKE_WEIGHT_MOUSE_OVER));
                    tmpPolyline.addPolylineMouseOutListener(mouseOutEvent -> tmpPolyline.setStrokeWeight(MapConstants.EDGE_STROKE_WEIGHT));
