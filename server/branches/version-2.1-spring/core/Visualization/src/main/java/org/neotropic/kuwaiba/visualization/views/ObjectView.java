 ...
 */
public class ObjectView extends AbstractDetailedView<BusinessObjectLight, VerticalLayout> {
    ...
    public ObjectView(BusinessObjectLight businessObject, BusinessEntityManager bem, ApplicationEntityManager aem, 
    ...
    public MxGraphCanvas<BusinessObjectLight, BusinessObjectLight> getMxGraph() {
        return mxGraph;
    }
    ...
    public byte[] getAsXml() {
        try {
            ...
            xmlew.add(xmlef.createAttribute(new QName("zoom"), mxGraph.getMxGraph().getScale() + "")); //NOI18N
...
//            } catch(Exception e) { }
    ...
    public VerticalLayout getAsComponent() throws InvalidArgumentException {
        ...
        VerticalLayout lytObjectView = new VerticalLayout(mxGraph.getMxGraph());
        ...
        for (AbstractViewNode bObject : viewMap.getNodes()) {
            ...
            node.setLabel(bObject.getIdentifier().toString());
            node.setGeometry(((int)(bObject.getProperties().get("x"))), (int)(bObject.getProperties().get("y")), 80, 20);
            ...
            mxGraph.addNode((BusinessObjectLight) bObject.getIdentifier(),node);
        } //        
                ...
                edge.setLabel(((BusinessObjectLight) anEdge.getIdentifier()).getName());
                 ...
                 if (sourceNode != null) {
                    ...
                    if (targetNode != null) { //The edge is only added if both sides are found
                        ...
                        mxGraph.addEdge((BusinessObjectLight) anEdge.getIdentifier(), 
   ...
   //          <editor-fold defaultstate="collapsed" desc="uncomment this for debugging purposes, write the XML view into a file">
                    ...
                    if (reader.getName().equals(qViewPosition)) {
                        try {
                            if (reader.getAttributeValue(null, "zoom") != null) {
                                ...
                                mxGraph.getMxGraph().setScale(zoom);
                            }
         ...
         try {
            ...
            if (!nodeChildren.isEmpty()) { 
                ...
                List<BusinessObjectLight> connectionChildren = bem.getSpecialChildrenOfClassLight(asLocalBusinessObject.getId(), 
                        asLocalBusinessObject.getClassName(), Constants.CLASS_GENERICPHYSICALLINK, -1);
            ...
            if (!objectViews.isEmpty()) 
                this.buildWithSavedView(aem.getObjectRelatedView(asLocalBusinessObject.getId(), 
                        asLocalBusinessObject.getClassName(), objectViews.get(0).getId()).getStructure()); 
    ...
    public void syncViewMap() {
        ...
        for (Map.Entry<BusinessObjectLight, MxGraphNode> entry : mxGraph.getNodes().entrySet()) {
        ...
        for (Map.Entry<BusinessObjectLight, MxGraphEdge> entry : mxGraph.getEdges().entrySet()) {
            ...
            this.viewMap.attachSourceNode(anEdge, new BusinessObjectViewNode(mxGraph.findSourceEdgeObject(entry.getKey())));
            this.viewMap.attachTargetNode(anEdge, new BusinessObjectViewNode(mxGraph.findTargetEdgeObject(entry.getKey())));
        }
    }
