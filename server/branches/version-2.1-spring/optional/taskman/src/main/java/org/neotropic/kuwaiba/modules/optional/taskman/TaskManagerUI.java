    ...
    private class TaskForm extends VerticalLayout {
        public TaskForm(Task task) {
            ...
            btnCommit.addValueChangeListener(event -> {    
               ...
               try {
                    aem.updateTaskProperties(task.getId(), Constants.PROPERTY_COMMIT_ON_EXECUTE, Boolean.toString(event.getValue()));
                } catch (ApplicationObjectNotFoundException | InvalidArgumentException ex) {
                    Logger.getLogger(TaskManagerUI.class.getName()).log(Level.SEVERE, null, ex);
                }
            });
            ...
            btnExecuteTask.setClassName("icon-button");
            ...
            lytHeaderButton.setPadding(false);
            lytHeaderButton.setMargin(false);
            ...
            lytTaskForm = new VerticalLayout(lytHeader, lytScript);
            lytTaskForm.setHeightFull();
            lytTaskForm.setMargin(false);
            lytTaskForm.setPadding(false);
            lytTaskForm.setSpacing(false);
            add(lytTaskForm);   
        }
    }
            ...
            btnSave.addClickListener(event -> {
                try {
                    try {
                        ...
                        aem.updateTaskProperties(task.getId(), Constants.PROPERTY_ENABLED, Boolean.toString(task.isEnabled()));
                        aem.updateTaskProperties(task.getId(), Constants.PROPERTY_COMMIT_ON_EXECUTE, Boolean.toString(task.commitOnExecute()));
                    } catch (InvalidArgumentException ex) {
                        Logger.getLogger(TaskManagerUI.class.getName()).log(Level.SEVERE, null, ex);
                    }
