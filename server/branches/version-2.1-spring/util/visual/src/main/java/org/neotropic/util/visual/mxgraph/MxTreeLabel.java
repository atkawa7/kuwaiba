...
/**
 * Show a circle with a color and a label
 * +-----------------+
 * | +---+ +-------+ |
 * | | O | | label | |
 * | +---+ +-------+ |
 ...
 */
public class MxTreeLabel extends MxGraphNode {
    ...
    public MxTreeLabel(MxGraph graph, String id, String label, String color) {
        super();
    ...
    private String getLabel(String label) {
        if (label != null && label.length() > CHARACTER_LIMIT)
            label = String.format("%s ...", label.substring(0, CHARACTER_LIMIT + 1));
        return label;
    }
}
