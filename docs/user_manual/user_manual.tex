...
\begin{document}
	...
	Layouts are used to show how elements look like in the real world. For example, you can design a layout of the front panel of a router model, with its slots, ports, lights, screws, etc. The layouts you design in Kuwaiba are dynamic, so what you do, is to create the graphic elements and name them after the expected children of the object, then the rendering algorithm matches everything and shows the view, either as a stand-alone one, or inside a conventional rack view. You can also create reusable shapes that can be shared across different layouts (like say, RJ45 ports). This shapes are called \textit{Custom Shapes}.
	
	It is important to note that the device layouts are only available for instances of subclasses of \textbf{GenericCommunicationElement} (routers, switches, servers, etc) and \textbf{GenericFrame} (ODFs and DDFs). Also, it's mandatory that the classes have a list type attribute called \textbf{model}.
	%\begin{figure}[h!]
	%	\centering
	...
	A layout is a set of basic shapes (squares, circles, text, triangles, etc) and custom shapes (compositions of basic shapes) that represent the physical appearance of a device, and in most of the cases, devices that are are to be installed inside a rack. If the layout is designed correctly, it can be embedded in a\textbf{\nameref{sec:rack_view}}.
	\newpage
