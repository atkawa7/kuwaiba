...
\begin{document}
	...
	\section{SDH Networks}
		...
		\begin{appendices}
			...
			Depending on the combination of patterns, only a limited number of names can be generated. If the number of possible names is lesser than the number of objects to be created provided by the user, the objects or special objects will not be created.
			\appendix
			\section{Appendix B. Connector Matching} \label{app:AppendixB}
			It's possible to define matching rules for physical connections. These rules are based on the type of connectors of both the links and the ports. To add and re
		\end{appendices}				
\end{document}