...
\begin{document}
	...
	\begin{table}[h!]
		...
		\begin{tabular}{cl}
			...
			\midrule
			\includegraphics[width=0.5cm]{img/icon_relationship_explorer.png} & Show objects relationships\\
			\midrule
			\includegraphics[width=0.5cm]{img/icon_special_relationship_explorer.png} & Show objects special children\\
			\midrule
			...
			\midrule
			\includegraphics[width=0.5cm]{img/icon_contract_manager.png} & Manage client, contracts and resources associated to them\\
			\midrule
			\includegraphics[width=0.5cm]{img/icon_IPAM_manager.png} & Manage Subnets and IP Addresses\\
			\midrule
			\includegraphics[width=0.5cm]{img/icon_SDH_networks.png} & Manage Synchronous Digital Hierarchy\\
			\midrule
			\includegraphics[width=0.5cm]{img/icon_MPLS_mudule.png} & Multiprotocol Label Switching module\\
			\midrule
			\includegraphics[width=0.5cm]{img/icon_projects.png} & Manage client, projects and resources associated to them\\
		\end{tabular}	
	...
	\section{Data Model Manager} \label{sec:data_model_manager}
		...
		\subsection{Class Tree}
			...
			\begin{itemize}
				...
				\begin{framed} {\large \textbf{Important}}
					\begin{itemize}
						...
						\item As a convention, all abstract classes have the prefix \textbf{Generic}. Note that a few core classes (like \textbf{InventoryObject} or \textbf{AdministrativeItem}) are abstract and are the exception to this rule. You, however, should try to follow this convention as much as possible.
					\end{itemize}
				\end{framed}
			...
			\newline \textbf{mandatory}, if is marked, it means that every new object of this class should has a value in this attribute, or if there are already created objects from this class, every object from the class need a value in this attribute, before you can mark it as mandatory.  
			\newline \textbf{unique}, if its marked, it means that the value of this attribute can not be repeated across all the  created objects from this class or its subclasses.  Before you can set a class attribute as unique, you should check if the value of this attribute in every created object from this class or its subclases is unique.
			\newline \textbf{visible}, this manage the attribute  visibility, if is not marked the attribute will not be shown in the property sheets of the objects created from this class.  
	...
	The special containment hierarchy (see figure~\ref{fig:special_containment_manager}) is used to define the business rules for special children see \textbf{\nameref{sec:extra_explorers_children_explorer}}. For example a cable inside a conduit connecting two buildings: in this scenario to configure the hierarchy for a wire container drag links like (ElectricalLink, OpticalLink, PowerLink, RadioLink or for simply used the GenericPhysicalLink) from the right panel to the WireContainer class in the left panel, and in this way using the \textbf{\nameref{sec:extra_explorers_children_explorer}} you can create special children for a conduit.
	...
	\begin{itemize}
		...
		\item \textbf{Create Physical Connection:} Creates a physical connection between two nodes selected in the Navigation Tree. Is disabled if only one node is selected. See more details in the chapter \textbf{\nameref{sec:physical_connections}}.
		...
		\item \textbf{Show Rack View:} Opens the Rack view to the selected object. See more details in the chapter \textbf{\nameref{sec:object_view_and_rack_view}}.
		...
		\item \textbf{Relate to Project:} All inventory objects can be a resource of an existing Project. See more details in the chapter \textbf{\nameref{sec:projects}}
		\item \textbf{Release from Project:} Removes a selected object (resource) form a Project.
	...
	\subsubsection{Relationship Explorer} \label{sec:extra_explorers_relationship_explorer}
		...
		\begin{table}[h!]
			...
			\begin{tabular}{cl}
    			...
    			\midrule
	 			\includegraphics[width=0.5cm]{img/icon_refresh_view.png} & Refresh the object special relationships\\
				\midrule
			\end{tabular}
	...
	You can also create templates from special objects (see \textbf{\nameref{sec:extra_explorers_children_explorer}}). The Figure \ref{fig:template_with_special_objects} shows the template for a Non-Armored Loose Tube Cable. 
	
	\begin{figure}[h!]
		\centering
		\includegraphics[width=0.8\linewidth]{img/template_with_special_objects.png}
	...
	\begin{figure}[h!]
		...
		\caption{Shown Object View and Show Rack View actions to the selected \textbf{Rack} object}
		\label{fig:actions_object_and_rack_views}
	\end{figure}
	\subsection{Object View} \label{sec:default_view}
		A view is a graphical representation of an object. There are many types of views, because an object has different perspectives, for example, a service object may have a view showing all the resources associated to it, a second view showing how all those resources are connected and a third showing statistics about such service. All instances (objects) of subclasses of \textbf{ViewableObject} have a Object View that displays the direct children of the selected node. Most objects, except logical and administrative assets and a few physical ones such as slots and ports are subclasses of \textbf{ViewableObject}. To access this view, select a node in the navigation tree right-click on it and click on the action Show Object View.
		\begin{figure}[h!]
			\centering
		...
		\begin{table}[h!]
			...
			\begin{tabular}[h!]{lp{10cm}}
				...
				\midrule
				\includegraphics[width=0.5cm]{img/object_view_high_contrast.png} & Node labels high contrast\\
				\midrule
				\includegraphics[width=0.5cm]{img/icon_select_tool.png} & Select/Move tool (selected by default)\\
				\midrule
				\includegraphics[width=0.5cm]{img/object_view_container.png} & Create connections using container (See \textbf{\nameref{sec:physical_connections}} for more details on how to use it)\\
				\midrule
				\includegraphics[width=0.5cm]{img/object_view_link.png} & Create connection using links (See \textbf{\nameref{sec:physical_connections}} for more details on how to use it)\\	
				\midrule
	...
	\section{Physical Connections} \label{sec:physical_connections}
		...
		\subsection{Example 2}	
			...
			\begin{enumerate}
					...
					\begin{framed} {\large \textbf{Important}}
						\begin{itemize}
							\item In optical connections, Rx and Tx ports are treated as a single port and a pair of fibers, actually represented as a single \textbf{OpticalLink}.							
						\end{itemize}
					\end{framed}
					...
					\end{figure}
					\begin{framed} {\large \textbf{Important}}\\
							Remember to use an \textbf{OpticalLink} instead of an \textbf{ElectricalLink}.							
					\end{framed}
	...
	\section{SDH Networks}
		...
		\section{Projects}\label{sec:projects}
			...
			The Project module allows you to have all projects, activities and resources in a place. To open the Projects module look at the menu for Tools $\rightarrow$ Advanced $\rightarrow$ Projects. To start using the Projects module you must create a Pool of Projects. Depending on the loaded data model you can create a  GeneralPurposeProject or NetworkProject or any subclass of GenericProject (see \textbf{\nameref{sec:data_model_manager}} if you need to create a different kind  of Project).
		...
		\begin{appendices}
			...
			When name patterns are defined, this can only generate a limited number of names. If in the dialogs see figure~\ref{fig:new_multiple} and see figure~\ref{fig:new_special_multiple}, the number assigned is greater than the possible names, the objects or special objects are not created.
