...
\begin{document}
	...
	\begin{itemize}
		...
		\item \textbf{New (Multiple):} Crates a defined number of objects at once using a given name pattern see \textbf{\nameref{app:AppendixA}} figure~\ref{fig:new_multiple}.
			\begin{figure}[h!]
				\centering
	...
	\subsubsection{Relationship Explorer} \label{sec:extra_explorers_relationship_explorer}
		...
		\begin{itemize}
			...
			\end{figure}			
			\item \textbf{New Special from template:} Crates a containment structure. See more details in the chapter \nameref{sec:template_manager}
		\end{itemize}
	...
	Also is available create template using special objects see \textbf{\nameref{sec:extra_explorers_children_explorer}}, in the \ref{fig:template_with_special_objects} show the template for a Non-Armored Loose Tube Cable. 
	...
	\end{figure}
	Copy, paste and rename operations work as they do with normal inventory objects and you can use them to construct your templates. Remember that you have to configure correctly the standard (see \ref{fig:standard_containment_manager}) and special (see \ref{fig:special_containment_manager}) containment hierarchy  before creating templates.\\
	...
	\subsection{Physical View} \label{sec:default_view}
		...
		In the Rack View you have an toolbar to export the view or refresh the last change in the view.
	...
	\section{SDH Networks}
		...
		\section{Projects}\label{sec:projects}
		    ...
		    To start using the Projects module you must crate a Project, depending of the loaded data model you can create project of GeneralPurposeProject or NetworkProject or any subclass of GenericProject (see \textbf{\nameref{sec:data_model_manager}} if you need to create a different kind  of Projects).
		    \newline
		...
		\begin{appendices}
			...
			A name pattern is used to create dynamically the names of new objects or new special objects using the actions New (Multiple) and New Special (Multiple) respectively.			
			
			The name pattern is a set of static and dynamic sections, in the static section can be place any string of characters with the exception of the special characters "[" and "]", and in the dynamic section only can be placed pre-defined functions listed below.
			
			\begin{itemize}
				\item \textbf{[sequence(a,z)]:} Function that generate an alphabetic ascending lowercase inclusive sequence that begin in the parameter a (a-z) to the parameter z (a-z)
				\item \textbf{[sequence(A,Z)]:} Function that generate an alphabetic ascending uppercase inclusive sequence that begin in the parameter A (A-Z) to the parameter Z (A-Z)
				\item \textbf{[sequence(n,m)]:} Function that generate an numeric ascending inclusive sequence that begin in the parameter n (0...) to the parameter m (0...)
				\item \textbf{[value(object id,attribute name)]:} Function to get the value of an attribute value given the object id and attribute name. For example if there are an object with id 1000, attribute name "name" and attribute value "Colombia" the function would remain [value(1000,name)] and return Colombia.
			\end{itemize}
			
			Examples of name pattern are: router-[sequence(1,3)]-[sequence(a,c)], [value(1111,name)]-[sequence(A,Z)], the first pattern generate six different names, and the second twenty six different names.
			
			When name pattern are define this only can generate a limited number of names. If in the dialog see figure~\ref{fig:new_multiple} and see figure~\ref{fig:new_special_multiple}, is assigned a number greather than the possible names, the objects or special objects are not created.
