		...
		\begin{appendices}
			...
			\section{Appendix B. How to Configure a Secure Connection} \label{app:AppendixB}
				...
				\begin{enumerate}
					...
					\textbf{Important:} If your server doesn't have a formal canonical name, and instead you reach it directky usin its IP address, the command needs and extra parameter:\\
			...
			Since version 1.5, the Kuwaiba client can be updated using an update center. An \textbf{Update Center} is a set of files which are put in a server to be downloaded by the client application when an update is available. To deploy the updates, unzip the updates.zip (included in the installation bundle), copy the files to a location in the server serving the updates that can be reached via HTTP/HTTPS and identify the URL to retrieve the \textbf{updates.xml} file; this URL must be used to configure the client, so it can sync the updates at startup as seen on \textbf{Figure~\ref{fig:update_center_options}}.
