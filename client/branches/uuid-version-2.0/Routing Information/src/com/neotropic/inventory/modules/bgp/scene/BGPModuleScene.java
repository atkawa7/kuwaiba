    ...
    public byte[] getAsXML() {
        try {
            ...
            for (Widget nodeWidget : nodeLayer.getChildren()) {
                ...
                if(nodeObject.getClassName().equals("Peering"))
                    xmlew.add(xmlef.createAttribute(new QName("peerings"), String.join("~", ((EmptyNodeWidget)nodeWidget).getExtraInfo())));
...
//</editor-fold>
            ...
            while (reader.hasNext()) {
                ...
                if (event == XMLStreamConstants.START_ELEMENT){
                    if (reader.getName().equals(qNode)){
                        ...
                        String peerings = reader.getAttributeValue(null, "peerings");
                        ...
                        if(objectClass.equals("Peering"))
                            lol = new LocalObjectLight(objectId, peerings, objectClass);
                        else
                        ...
                        if (reader.getName().equals(qEdge)){
                            ...
                            LocalObjectLight container;
                            if(className.isEmpty())
                                container = new LocalObjectLight(objectId, "", "");
                            else
                                container = CommunicationsStub.getInstance().getObjectInfoLight(className, objectId);
                            if (container != null) {
    ...
    public void createBGPView(List<LocalLogicalConnectionDetails> bgpMap){
        ...
        for (Map.Entry<LocalObjectLight, List<LocalObjectLight>> entry : devicePeerings.entrySet()) {
            ...
            LocalObjectLight peering = new LocalObjectLight(r.nextLong(), peeringsNames, "Peering");
            addNode(peering);
            validate();
            LocalObjectLight tempE = new LocalObjectLight(r.nextLong(), "", "");
            addEdge(tempE);
            validate();
