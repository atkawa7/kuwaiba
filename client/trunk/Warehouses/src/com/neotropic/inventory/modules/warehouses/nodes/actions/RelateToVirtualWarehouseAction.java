...
/**
 * This action allows the user relate the current object to a Virtual Warehouse as a resource
 * @author Johny Andres Ortega Ruiz <johny.ortega@kuwaiba.org>
 */
@ActionsGroupType(group=ActionsGroupType.Group.RELATE_TO)
@ServiceProvider(service=GenericObjectNodeAction.class)
public class RelateToVirtualWarehouseAction extends GenericObjectNodeAction implements ComposedAction {
    
    public RelateToVirtualWarehouseAction() {
        putValue(NAME, I18N.gm("relate_to_virtual_warehouse"));
    }
   
    @Override
    public void actionPerformed(ActionEvent e) {
        
        List<LocalObjectLight> virtualWarehouses = CommunicationsStub.getInstance().getObjectsOfClassLight(Constants.CLASS_VIRTUALWAREHOUSE);

        if (virtualWarehouses ==  null)
            NotificationUtil.getInstance().showSimplePopup(I18N.gm("error"), NotificationUtil.ERROR_MESSAGE, CommunicationsStub.getInstance().getError());
        else {
            if (virtualWarehouses.isEmpty()) {
                JOptionPane.showMessageDialog(null, "There are no virtual warehouses created. Create at least one using the Warehouses Manager", 
                ...
                SelectValueFrame frame = new SelectValueFrame(
                    ...
                    "Create Relationship", virtualWarehouses);
                ...
                if (CommunicationsStub.getInstance().associatesPhysicalNodeToWarehouse(
                    ...
                    ((LocalObjectLight) selectedValue).getId())){
                        JOptionPane.showMessageDialog(null, String.format(selectedObjects.size() > 1 ? 
                                "%s obejcts were related to virtual warehouse %s" : "%s object was related to virtual warehouse %s", selectedObjects.size(), selectedValue));
                        frame.dispose();
                }
                else 
