...
public class LabelShapeWidget extends LabelWidget  implements PropertyChangeListener, SharedContentLookup {
    ...
    public void propertyChange(PropertyChangeEvent evt) {
        ...
        } else if (Shape.PROPERTY_BORDER_WIDTH.equals(evt.getPropertyName())) {
            if (evt.getSource() instanceof Shape) {
                setBorder(BorderFactory.createLineBorder(
                    (Integer) evt.getNewValue(), 
                    ((Shape) evt.getSource()).getBorderColor()));
            }
        } else if (Shape.PROPERTY_BORDER_COLOR.equals(evt.getPropertyName())) {
            if (evt.getSource() instanceof Shape) {
                setBorder(BorderFactory.createLineBorder(
                    ((Shape) evt.getSource()).getBorderWidth(), 
                    (Color) evt.getNewValue()));
            }
        }        
        else if (LabelShape.PROPERTY_LABEL.equals(evt.getPropertyName())) {            
