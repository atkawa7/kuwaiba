...
/**
 * Deletes objects of class Provider, used to group BGPPeers created with sync
 * @author Adrian MArtinez Molina {@literal <adrian.martinez@kuwaiba.org>}
 */
@ServiceProvider(service = GenericObjectNodeAction.class)
public class DeleteProviderAction extends GenericObjectNodeAction implements Presenter.Popup{
    
    public DeleteProviderAction() {
        putValue(NAME, "Delete Provider");
    ...
    public void actionPerformed(ActionEvent e) {
        if (JOptionPane.showConfirmDialog(null, 
                ...
                "Delete Provider", JOptionPane.OK_CANCEL_OPTION, JOptionPane.WARNING_MESSAGE) == JOptionPane.OK_OPTION) {
            for (LocalObjectLight selectedObject : selectedObjects) {
                if (CommunicationsStub.getInstance().deleteProvider(selectedObject.getId())) 
